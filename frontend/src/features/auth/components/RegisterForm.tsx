...
export const RegisterForm: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <h2 className="mt-1 text-3xl font-semibold text-ink">Create account</h2>
      <p className="mt-2 text-sm text-ink-dim">Start planning your next adventure.</p>

      <label className="mt-6 block text-sm font-medium text-ink-dim">Full name</label>
      <div className={[
        'relative mt-2 rounded-[12px] border-[1.5px] bg-white dark:bg-[#16142e] transition-all',
        errors.name ? 'border-red-400' : 'border-sky-200 dark:border-[rgba(139,92,246,0.25)] focus-within:border-indigo-400 dark:focus-within:border-violet-500 focus-within:shadow-[0_0_0_3px_rgba(99,102,241,0.12)]',
      ].join(' ')}>
      ...
      <label className="mt-4 block text-sm font-medium text-ink-dim">Email</label>
      <div className={[
        'relative mt-2 rounded-[12px] border-[1.5px] bg-white dark:bg-[#16142e] transition-all',
      ...
      <p className="mt-5 text-center text-xs text-ink-faint">
        ...
        <Link to={ROUTES.LOGIN} className="text-indigo-500 hover:text-indigo-700 font-medium">
          Sign in
        </Link>
      </p>
    </form>
