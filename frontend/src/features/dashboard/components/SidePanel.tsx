  ...
  <motion.div initial="hidden" animate="show" variants={stagger} className="flex flex-col gap-5">
    <motion.section variants={fadeUp} className="rounded-2xl p-4 bg-white dark:bg-[#16142e] shadow-sm">
      <div className="flex items-center justify-between mb-3.5">
        <h3 className="text-sm font-bold text-blue-950 dark:text-blue-100">Active groups</h3>
        <Link to={ROUTES.GROUPS} className="text-xs font-semibold text-blue-600">View all</Link>
      </div>
      {groups.length === 0 ? (
